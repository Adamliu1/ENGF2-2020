...
#for data testing
#start_time = time()
#data_file = open("game_dataout.out", "w")
#data_file_1 = open("game_dataout_1.out", "w")
...
class Bomb():
    ...
    def move(self,):
        if self.falling:
            ...
            # one second is 60fps, each frame acc = 9.81/60
...
class Plane():
    ...
    def move(self):
        ...
        if self.position.getX() < -self.width:
            #Bug 5: Because we only check speed per 10fps, hence the plane might not start outside canvas in next line.
            #Hence apply +100 to make sure plane starts outside canvas in order to drop bomb to bottom-right buildding.
...
class Display(Frame):
    ...
    def create_buildings(self):
        ...
        #create the new ones
        # Bug 2 : in range 1 + canva // spacing      1200 will produce building outside canva
    ...
    def check_bomb(self):
        ...
        # Bug 1: (fix)if the y axis of the bomb is at the bottom, then explode
    ...
    def check_plane(self):
        ...
        # Bug 3: if plane is at the bottom, ignore collision and only if at the bottom then when x <20, plane is landed. 
    ...
    def restart(self):
        ...
        #Bug 4 : Should only clean text if game has text (initial self.text as an empty string)
...
class Game():
    ...
    def checkspeed(self):
        ...
        if self.framecount == 1:
            ...
            self.lastframe = now
            self.framecount = 0
            #data_file_1.write("%s %s\n" % (time_passed, elapsed * 10))
            # for data testing (plot relation)
            #data_file.write("%s %s\n" % (time_passed, speed))
