...
class Adam_4(Player):
    ...
    def score_board(self, clone):
        ...
        #test new weighting 
        # test 1 | (3.9181268101392694) * erodedPieceCellsMetric (about 30k)
        # test 2 | (4.4181268101392694) * erodedPieceCellsMetric (27k)
        # test 3  | (4.4181268101392694) * erodedPieceCellsMetric + (-4.500158825082766) * (23-min_y) (about 29k) (tuning test 1 to gitlab)
        # test 4 | (-4.899265427351652) * hole_num + (5.4181268101392694) * erodedPieceCellsMetric + (-4.500158825082766) * (23-min_y) (about 27k)
        # test 5 | score = (-3) * transition_row + (-9) * transition_col \ + (-3) * hole_num + (-3.5) * boardwells
        # test 6 |         score = (-3) * transition_row + (-1) * transition_col \ + (-2.5) * hole_num + (-3.5) * boardwells (about 32k) (pushed test 2)
        # test 7 |         score = (-0.5) * transition_row + (-1) * transition_col \ + (-2.5) * hole_num + (-3.5) * boardwells still 31k
        # test 8 |         score = (-1) * transition_col \ + (-2.5) * hole_num + (-3.5) * boardwells (about 33k) (push test 3)
        # test 9 | score = (-3) * transition_row + (-1) * transition_col + (-2.5) * hole_num (push test 4)
        # test 10 | score = (-3) * transition_row + (-0.65) * transition_col + (-2.5) * hole_num (local about 35k) (push test 5)
        
        score = score = (-0.8) * transition_col + (-2.5) * hole_num + (-3.5) * boardwells
...
# Rotation -> Movement + next block + yiled
class Adam_5(Player):
    ...
    def rotation_first_next_block(self, board):
        ...
        for i in range(4):
            ...
            if not res:
                ...
                score = self.score_board_next(clone_board)
    ...
    def Priority(self, board):
        ...
        current_pos = board.falling.left
        return current_pos
    
    def getAvg(self, board):
        count = 0
        heights = []
        for i in range(10):
            count = 0
    ...
    def score_board(self, clone):
        ...
        cells = clone.cells
        #if cells != False:
        #check if list is empty + get average height
        if cells:
            min_y = min([y for x,y in cells])
        ...
        transition_col = self.boardcolTransitions(cells)
        #not yet (checked)
        row_eliminated = self.get_erodedPieceCellsMetric(clone, cells)
        #checked
        hole_num = self.getBoardBuriedHoles(cells, min_y)
        #checked
        boardwells = self.getBoardWells(cells, min_y)
        priority = self.Priority(clone)
        avg = self.getAvg(clone)
        #print(hole_num)
        #print(clone.cells)
        #weithting
        ...
        #new strategy (add a safe height)
        if min_y < (0.43 * 24):
            if avg < (0.36 * 24):
                score = (-3) * transition_row + (-0.95) * transition_col + (-2.5) * hole_num + (-3.5) * boardwells + (-5) * (23-min_y)
            else:    
                score = (-3) * transition_row + (-0.95) * transition_col + (-2.5) * hole_num + (-3.5) * boardwells + (-0.5) * (23-min_y)
        else:
            score = (-3) * transition_row + (-0.95) * transition_col + (-2.5) * hole_num + (-3.5) * boardwells + (-0.3) * row_eliminated
    ...
    def score_board_next(self, clone):
        ...
        score = (-3) * transition_row + (-0.95) * transition_col + (-2.5) * hole_num + (-3.5) * boardwells + (-0.2) * row_eliminated
