...
class RiverObject():
    ...
    def contains(self, frog):
        ...
        #Bug 6: x,y coordiante of picture is 
        #print(self.get_width())
        #if frog_y != self.y or frog_x < self.x - GRID_SIZE//2 or frog_x > self.x - GRID_SIZE//2 + self.width:
...
class Model():
    ...
    #Improve1: remain time is 0, game over
    ...
    def create_logs(self):
        ...
        y = GRID_SIZE*4
        speeds = [1, 1, 2.5, 1, 2]
        # False means this is a log, True means a turtle
    ...
    def new_life(self):
        #Bug 2: didn't reset postion of frog after a frog is dead
    ...
    def move_frog(self, dir):
        if self.game_running and not self.paused:
            # Bug3: Movement should Only done by one (should not allow combine key)
            if self.frog.moving:
                return
            self.frog.move(dir)
        ...
        if on_log is None:
            ...
            for log in self.logs:
                if log.contains(self.frog):
                    #Bug 1: Variable name on_log was typed as on_long
                    #on_long = log
                    on_log = log
                    break
        if on_log is None:
    ...
    def check_frog(self):
        ...
        #Bug 4: Shoud not allow frog to move below start position
